...
#RENDER HTML REPORT & SHARE VIA EMAIL
knitr::knit2html("lvmpd_record_requests_markdown_FINAL.Rmd",options="")
...
#Email the output HTML file in the body of the email
send.mail(from = "sdavidson@reviewjournal.com",
          to = c("crimeteam@reviewjournal.com", "tlane@reviewjournal.com"),
          subject = paste0("LVMPD Record Requests - Export: ", ExportDateTime),
          body = "lvmpd_record_requests_markdown_FINAL.html",
          html = TRUE,
          inline = TRUE,
          smtp = list(host.name = "smtp.gmail.com", port = 465, 
                      user.name = "sdavidson@reviewjournal.com", 
...
unlink("lvmpd_record_requests_markdown_FINAL.html")
unlink("lvmpd_record_requests_markdown_FINAL.md")
...
drive_put(csv_path_join, 
          name = 
            paste0("lvmpd_pr_all_", currentDateTime), 
          type = "spreadsheet", 
          path=as_id(td))

#Import the MEDIA scrape into Google Drive for historical purposes
drive_put(csv_path_media, 
          name = 
            paste0("lvmpd_pr_media_", currentDateTime), 
          type = "spreadsheet", 
          path=as_id(td))
...
lvmpd_pr_master_path <- "data/lvmpd_pr_master.csv"
 
#Load in Master File from repository
lvmpd_pr_master <- read_csv("data/lvmpd_pr_master.csv") %>%
  mutate_all(as.character)

#Make all the data "characters"                      
lvmpd_join_master <- lvmpd_join_master %>%
  mutate_all(as.character)

#Rbind the newest results to it
lvmpd_pr_master_new <- rbind(lvmpd_join_master,
                             lvmpd_pr_master) %>% 
  #Delete any repeat requests
  distinct(request_id, .keep_all = TRUE)

#Write new CSV for repository
write.csv(lvmpd_pr_master_new, 
          lvmpd_pr_master_path, 
          row.names=FALSE)

#Upload that Master CSV to Google drive
drive_put(lvmpd_pr_master_path, 
          name =  "lvmpd_pr_master", 
          type = "spreadsheet", 
          path=as_id(td))
